...
public class TipoEquipamentoResource {
    ...
    public Response save(TipoEquipamento tp) {

        var tipoEquipamento = TipoEquipamentoRepository.save(tp);

        final URI tipoEquipamentoURI = UriBuilder
                .fromResource(TipoEquipamentoResource.class)
                .path("/{id}")
                .build(tipoEquipamento.getId());

        return Response.created(tipoEquipamentoURI).entity(tipoEquipamento).build();
    }
